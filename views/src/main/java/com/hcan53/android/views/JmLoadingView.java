 ...
 * @date 2016-09-21
 */
public class JmLoadingView extends View {
    ...
    private static final int DEFAULT_SIZE     =  32;  //默认loading view的大小
    private static final int LINE_COUNT       =  12;
    private static final int DEGREE_PER_LINE  =  360 / LINE_COUNT;

    public JmLoadingView(Context context) {
        this(context, null);
    }

    public JmLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public JmLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.JmLoadingView);
        mSize = array.getDimensionPixelSize(R.styleable.JmLoadingView_loading_view_size, DensityUtils.dp2px(context, DEFAULT_SIZE));
        mPaintColor = array.getInt(R.styleable.JmLoadingView_android_color, Color.WHITE);
        array.recycle();

        initPaint();
    }

    public JmLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
