...
public class NotificationUtils extends ContextWrapper {
     ...
     * 发送带有进度的通知
     */
    public void sendNotificationProgress(int notity, String title, String content, int progress, PendingIntent intent) {
        builder = getNotificationProgress(title, content, progress, intent);
        builder.setOngoing(true);
        getManager().notify(notity, builder.build());
    }
    ...
    public void setIntent(int notity, PendingIntent intent) {
        if (builder != null) {
            builder.setProgress(100, 100, false);
            builder.setContentText("下载完成");
            builder.setContentIntent(intent);
     ...
     * 清除进度通知
     */
    public void clearNoticeLoad(int notity) {
        getManager().cancel(notity);
    }
